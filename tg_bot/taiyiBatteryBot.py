...
# coding=utf-8
import logging, sqlite3, json, sys, random, time
...
def wrapper(*args,**kwargs):
    def w(fn):
        def ww(bot,update):
            logging.info('user: {} command: {}'.format(update.message.from_user.username,update.message.text))
            text = fn(bot,update)
            logging.info('response: {}'.format(text))
            if kwargs.get('reply_to',None):
                kwargs['reply_to'] = update.message.message_id
            bot.sendMessage(update.message.chat_id,reply_to_message_id=kwargs['reply_to'],text=text,parse_mode=kwargs.get('parse_mode',None), disable_web_page_preview=kwargs.get('disable_preview',None))
            return 0
        return ww
    return w
